...
city <- "TOR"
cities <- c("WIN","CHI","MIN","DEN","TOR")
numPC <- 8
lagdays <- c(1,2,3,4)
predDay <- min(lagdays)

# keep only requested cities
cityData <- maxTemp[,unique(c("DATE",city,cities))]

# create data frame with the temperatures lagged
# by one through "lagdays" days
lagCityData <- lagData(maxTemp, days=lagdays)
...
title <- paste("Temperature prediction for ",city,
               ...
               " -",paste(lagdays, collapse=", -"),
               "\nand cities: ", paste(cities, collapse=", "),
               sep="")
